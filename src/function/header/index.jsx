...
const Header = ({ noBtn, noArrow, noClose, padding, title, search, wish, func, check, arrowUrl, closeUrl, update, write }) => {

  const navigate = useNavigate();

  return (
    <div className={'h-12 flex items-center justify-center bg-white relative border-b border-grey07' + (padding ? ' p-5' : '')}>
      ...
      <div className={'absolute' + (padding ? ' right-5' : ' right-0')}>
        {!noClose && !noBtn && !wish?.open && !search && !check && !update && !write && <img src="/svg/close.svg" style={{width: '18px'}}
          onClick={() =>
        ...
        {update && <div onClick={func}>수정</div>}

        {write && <div onClick={func}>완료</div>}
      </div>
    </div>
  )
}
